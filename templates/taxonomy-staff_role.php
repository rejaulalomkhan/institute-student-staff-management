<?php
/**
 * Staff Role Taxonomy Archive Template
 */

get_header();

// Handle both normal taxonomy and custom query var
$current_term = get_queried_object();
if (!$current_term || !isset($current_term->taxonomy)) {
    $role_slug = get_query_var('staff_role');
    if ($role_slug) {
        $current_term = get_term_by('slug', $role_slug, 'staff_role');
    }
}
$settings = get_option('institute_management_settings', array());
$institute_name = $settings['institute_name'] ?? get_bloginfo('name');

// Role-specific Bengali titles
$role_titles = array(
    'office-staff' => 'অফিস স্টাফ',
    'teachers' => 'শিক্ষক মণ্ডলী',
    'committee' => 'কমিটি বৃন্দ',
    'administration' => 'প্রশাসনিক কর্মকর্তা',
    'support-staff' => 'সহায়ক কর্মী'
);

$bengali_title = $role_titles[$current_term->slug] ?? $current_term->name;
?>

<div class="institute-archive-wrapper staff-role-archive">
    <div class="institute-archive-header">
        <div class="container">
            <h1 class="institute-archive-title">
                <span class="english-subtitle"><?php echo esc_html($current_term->name); ?></span>
            </h1>
            ...
            <div class="institute-display-controls">
                ...
                <div class="institute-compact-filters">
                    <div class="institute-filter-compact">
                        <label for="role-department-filter"><?php _e('Department:', 'institute-management'); ?></label>
                        <select id="role-department-filter" class="institute-compact-select">
                            <option value=""><?php _e('All Departments', 'institute-management'); ?></option>
                            <?php
                            // Get departments for current role
                    ...
                    <div class="institute-filter-compact">
                        <label for="role-status-filter"><?php _e('Status:', 'institute-management'); ?></label>
                        <select id="role-status-filter" class="institute-compact-select">
                ...
                <div class="institute-search-controls">
                    <label for="role-search-compact"><?php _e('Search:', 'institute-management'); ?></label>
                    <input type="text" id="role-search-compact" class="institute-search-input" placeholder="<?php _e('Search by name, ID, position...', 'institute-management'); ?>" />
                </div>
            </div>
            
            <!-- Results Area -->
            <div id="staff-role-results" class="institute-staff-results">
                ...
                <!-- Grid View -->
                <div class="institute-staff-grid institute-grid institute-columns-3" style="display: none;">
                ...
                <div class="institute-no-results">
                    ...
                    <h3><?php printf(__('No %s found', 'institute-management'), esc_html($current_term->name)); ?></h3>
                    <p><?php printf(__('There are currently no %s to display. Please check back later or contact the administration.', 'institute-management'), esc_html(strtolower($current_term->name))); ?></p>
                    
                    <?php if (current_user_can('edit_posts')): ?>
                    <p>
                        <a href="<?php echo admin_url('post-new.php?post_type=staff'); ?>" class="institute-btn institute-btn-primary">
                            <?php printf(__('Add %s', 'institute-management'), esc_html($current_term->name)); ?>
                        </a>
                    </p>
                    <?php endif; ?>
                </div>
...
<style>
.staff-role-archive .bengali-title {
    font-size: 2.5em;
    font-weight: bold;
    color: #ffffff;
    display: block;
...
.staff-role-archive .english-subtitle {
    font-size: 1.2em;
    color: #ffffff;
    display: block;
}
...
if (typeof institute_templates === 'undefined') {
    var institute_templates = {
        ajax_url: '<?php echo admin_url('admin-ajax.php'); ?>',
        nonce: '<?php echo wp_create_nonce('institute_templates_nonce'); ?>',
        strings: {
...
jQuery(document).ready(function($) {
    ...
    $('#role-search-compact').on('input', function() {
    ...
    function performRoleFilter() {
        var filters = {
            role: '<?php echo esc_js($current_term->slug); ?>',
            department: $('#role-department-filter').val(),
            status: $('#role-status-filter').val(),
            search: $('#role-search-compact').val()
        };
        loadRoleResults('role_filter', filters);
    }
    
    function loadRoleResults(action, data) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    $('#staff-role-results').html(response.data);
                } else {
